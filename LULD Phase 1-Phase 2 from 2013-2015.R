...
SP500= getSymbols("^GSPC", from = '2013-01-01',
...
n=length(log_ret)
#PHASE 1 LULD
e1=which(row.names(as.matrix(log_ret))>="2013-04-08"& row.names(as.matrix(log_ret))<="2013-05-31")
d1=rep(0,n)
d1[e1]=1
#PHASE 2 LULD
e2=which(row.names(as.matrix(log_ret))>="2013-08-05"& row.names(as.matrix(log_ret))<="2013-12-09")
d2=rep(0,n)
d2[e2]=1
d1
d2

df_dummies=matrix(c(d1,d2), ncol = 2)
...
residual_bestModel= residuals(e_garchFit, standardize = TRUE)
